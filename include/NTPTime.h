...
bool syncTime(void)
{
  ...
  if (nextSendTime < millis()) {
    ...
    if ( no_packet_count > 0 )  {
      ...
      // return false if sync was not successful
      return false;
    }
    else {
      // Wait 1 hour for next sync
  ...
  // return false, because there were no sync
  return false;
}
...
void decodeNTP(void)
{
  ...
  while (millis() < waitTime && !timeValid)
  {
    ...
    if (udp.parsePacket())
    {
      ...
      //pointer to current time change rule, used to get TZ abbrev
      local_time = euCET.toLocal(utc, &tcr);

      // Print the hour, minute and second:
      Serial.print("Received NTP UTC time : ");

      uint8_t yy = year(local_time);
      Serial.print(yy); // print the year

      uint8_t mo = month(local_time);
      Serial.print(mo); // print the month

      uint8_t dd = day(local_time);
      Serial.print(dd); // print the day
      ...
      if (yy >= 1000)
        actualNTPTime.Year = CalendarYrToTm(yy);
      else    // (y < 100)
        actualNTPTime.Year = y2kYearToTm(yy);
      actualNTPTime.Month = mo;
      actualNTPTime.Day = dd;
      actualNTPTime.Hour = hh;
      actualNTPTime.Minute = mm;
      actualNTPTime.Second = ss;
